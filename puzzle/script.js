...
function Menu(params) {
  ...
  let that = this;
  dt.addEventListener("click", () => that.toggle());
...
Menu.prototype.toggle = function () {
  if ("open" == this.divMenu.classList.value) this.collapse();
  else this.display();
};
...
let uploadFile;
{
  ...
  function getFile() {
    ...
    reader.addEventListener("load", () => {
      if (options.image) {
        options.image.src = reader.result;
      }
...
function Puzzle(params) {
  ...
  this.setAudio(params.audio);
  if (typeof params.img == "string") {
    this.setImage(params.img);
  } else {
...
Puzzle.prototype.setImage = function (imgSrc) {
  let params;
...
*/
  ...
  if (!this.menu) {
    // Create menu items from imgAudioList
    let lines = [{ text: "full screen", func: this.toggleFullScreen() }];
    for (const imgAudio of imgAudioList) {
      lines.push({
        text: imgAudio.text,
        func: this.changeImage(imgAudio.img, imgAudio.audio),
      });
    }
    this.menu = new Menu({
...
Puzzle.prototype.toggleFullScreen = function () {
  let puz = this;
  return function () {
    puz.menu.collapse();
...
Puzzle.prototype.changeImage = function (imgSrc, audioSrc) {
  let puz = this;
  return function () {
    puz.menu.collapse();
    puz.setImage(imgSrc);
    puz.pauseAudio();
    puz.setAudio(audioSrc);
  };
};
...
window.addEventListener("load", function () {
  new Puzzle({
    img: imgAudioList[0].img,
    audio: imgAudioList[0].audio,
